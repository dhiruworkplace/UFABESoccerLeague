using Assets.UFABESoccerLeague.Scripts.Managers;
using Assets.UFABESoccerLeague.Scripts.StateMachines.Managers;
using Assets.UFABESoccerLeague.Scripts.Utilities;
using RobustFSM.Base;
using UnityEngine;
using static Assets.UFABESoccerLeague.Scripts.Managers.MatchManager;

namespace Assets.UFABESoccerLeague_.Scripts.States.Managers.MatchManagerMainState.MatchOn.SubStates.MatchStopped.SubStates
{
    public class BroadcastNormalTimeIsOver : BState
    {
        /// <summary>
        /// A reference to the wait time
        /// </summary>
        float waitTime;

        public override void Enter()
        {
            base.Enter();

            //set the wait time
            waitTime = 1f;

            //raise the half-start event
            RaiseNormalTimeOverEvent();
        }

        public override void Execute()
        {
            base.Execute();

            //decrement the time
            waitTime -= Time.deltaTime;

            //go to wait-for-kick-to-complete if time is less than 0
            if (waitTime <= 0f) Machine.ChangeState<NormalTimeOver>();
        }

        public override void Exit()
        {
            base.Exit();

            //raise the event that I finished broadcasting normal time is over
            ActionUtility.Invoke_Action(Owner.OnFinishBroadcastMessage);
        }

        /// <summary>
        /// Raises the half start event
        /// </summary>
        public void RaiseNormalTimeOverEvent()
        {
            //prepare an empty string
            string message = "Normal Time Over";

            //raise the event
            ActionUtility.Invoke_Action(message, Owner.OnStartBroadcastMessage);
        }

        /// <summary>
        /// Returns the owner of this instance
        /// </summary>
        public MatchManager Owner
        {
            get
            {
                return ((MatchManagerFSM)SuperMachine).Owner;
            }
        }
    }
}